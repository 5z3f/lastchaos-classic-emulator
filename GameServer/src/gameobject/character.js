...
const { Statistic, Modifier, ModifierType, ModifierOrigin } = require('../types/statistic');
...
class Character extends util.extender(GameObject, Attackable) {
    ...
    calculateStatpoints() {
        ...
        var bonusHealth = calculateBonusHealth(
            this.classType,
            this.progress.level,
        ...
        var bonusMana = calculateBonusMana(
            this.classType,
            this.progress.level,
            this.statistics.strength.getCurrentValue(),
            this.statistics.dexterity.getCurrentValue(),
        ...
        var bonusAttack = calculateBonusAttack(
            this.classType,
            this.progress.level,
            this.statistics.strength.getCurrentValue(),
            this.statistics.dexterity.getCurrentValue(),
    ...
    calculateWearingItems() {
        ...
        var wearingRows = this.inventory.filter(0, (r) => r?.wearingPosition != 255 && r?.wearingPosition != undefined)

        for (var row of wearingRows) {
            switch (row.item.wearingPosition) {
                ...
                case WearingPosition.Weapon:
                    ...
                    if(this.progress.level < row.item.level) {
                        var levelDiff = row.item.level - this.progress.level;

                        if(levelDiff <= 4)
                            break;

                        var penaltyInPercent = (levelDiff > 12) ? .90 : (levelDiff > 8) ? .70 : (levelDiff > 4) ? .50 : 0;
