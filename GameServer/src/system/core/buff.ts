...
export class Buff extends EventEmitter<BuffEvents> {
    ...
    constructor(owner: Character, originType: BuffOrigin, originRef: string | BaseItem, modifiedStatistics?: ModifiedStatistic[], duration: number | string | undefined = null, script?: BuffScript) {
        ...
        this.owner = owner;
        this.duration = (typeof duration == 'string') ? util.durationToSeconds(duration)
    ...
    static fromItem(owner: Character, baseItem: BaseItem): Buff {
        ...
        let buffScript = getBuffScript(spellId);
    ...
    apply() {
        if(this.script) {
            if(this.originType === BuffOrigin.Item) {
                ...
                this.duration = (typeof this.script.duration == 'string') ? util.durationToSeconds(this.script.duration)
    ...
    remove() {
        ...
        if(this.originType === BuffOrigin.Item) {
            ...
            // search for buff script with given index
            let buffScript = getBuffScript(spellIndex);
...
export class Buffs {
    ...
    remove(buff: Buff | number, originType?: BuffOrigin, originRef?: string | BaseItem) {
        let isBuffClass = buff instanceof Buff;

        if(!isBuffClass)
            buff = this.find(originType, originRef);
